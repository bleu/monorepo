...
const convert = (key: string, value: string) => {
  if (['minTVL', 'maxTVL', 'minAPR', 'maxAPR'].includes(key)) return Number(value) || undefined;
  if (['tokens', 'type', 'network'].includes(key)) return value ? value.split(',') : undefined;
  return value;
};
function getFilterDataFromParams(searchParams: SearchParams) {
  return Object.fromEntries(
    Object.entries(searchParams).map(([key, value]) => [key, convert(key, value) || (key === 'minTVL' ? MIN_TVL : undefined)])
  );
}
export default function getFilteredApiUrl(searchParams: SearchParams) {
  ...
  const params = Object.entries(filteredData).map(([key, value]) => value !== undefined ? `&${key}=${value}` : "").join("");
