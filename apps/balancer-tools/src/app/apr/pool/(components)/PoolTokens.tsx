...
export default async function PoolTokens({
  ...
  const poolData: PoolStatsResults = await fetcher(
    `${BASE_URL}/apr/api/?poolId=${poolId}${roundId ? `&roundId=${roundId}` : ""}`,
  );
  if (roundId) {
  poolData.perRound.map(pool => {
    pool.tokens.map(async token => {
      ...
      // @ts-ignore 2345
      const tprice = await getTokenPriceByRound(Round.getRoundByNumber(roundId), token.address, pool.network)
      // @ts-ignore 2345
      token.price = tprice
    })
  })
