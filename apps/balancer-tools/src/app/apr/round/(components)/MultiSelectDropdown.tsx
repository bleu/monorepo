            ...
            {selectedItems.map((value, i) => {
                  return (
                      ...
                      >
                        ...
                        <span><Cross1Icon /></span>
                      </button>
                  );
                })}

              <input placeholder={labelText} className="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0" {...getInputProps()} type="text" />
              </div>
              </div>
              ...
              {isOpen && (
                ...
                {items
                    ...
                    .map(item => {
                      return (
                        <div className="flex w-full flex-col items-start cursor-pointer"
...
function changeHandler(
  ...
  setSelectedItems: React.Dispatch<React.SetStateAction<{ id: number; value: string; }[]>>,
  onSelectionItemsChange: (items: { id: number; value: string }[]) => void
) {
  return (selectedItem: { id: number; value: string } | null, downshift: ControllerStateAndHelpers<{ id: number; value: string }>) => {
