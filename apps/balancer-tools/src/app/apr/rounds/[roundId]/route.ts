...
export async function GET(
  ...
  const balWeeklyEmissions = getBalEmissions(selectedRound.endDate.getFullYear())['weekly']

  
  requestJson.forEach(poolData => {
    // APR = emissions in week * voting share * weeks in year * BAL price / TVL
    poolData['apr'] = ((balWeeklyEmissions * poolData.pct_votes * 52 * balPrice) / totalTvl) / 100
    return poolData
  });
