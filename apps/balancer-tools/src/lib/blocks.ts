...
const CHAIN_BLOCK_EXPLORER_FN_MAP: Record<Chain, (timestamp: number) => string> = {
    [Chain.gnosis]: (timestamp: number) => `https://api.gnosisscan.io/api?module=block&action=getblocknobytime&timestamp=${timestamp}&closest=before&apikey=${process.env.GNOSIS_API_KEY}`,
};
...
class Subgraph {
    static async bestGuess(chain: Chain = Chain.mainnet, t: number = this.getTime24hAgo()): Promise<number> {
        const response = await axios.get(CHAIN_BLOCK_EXPLORER_FN_MAP[chain](t));
        return parseInt(response.data.result);
    }

    static async getBlockNumberByTimestamp(chain: Chain = Chain.mainnet, timestamp: number = this.getTime24hAgo()): Promise<number> {
        const url = BLOCKS_SUBGRAPH_URL_MAP[chain];
        if (!url) {
            return this.bestGuess(chain, timestamp);
        }

        const data = await this.gql(url, BLOCKS_QUERY, this.getTimestamps(timestamp));
