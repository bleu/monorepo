...
import "./PoolMetadataAuthentication.sol";
import {BasePoolAuthorization} from "balancer-v2-monorepo/pkg/pool-utils/contracts/BasePoolAuthorization.sol";

abstract contract PoolMetadataAuthorization is PoolMetadataAuthentication {
    ...
    function _canPerform(bytes32 actionId, bytes32 poolId, address account) internal view override returns (bool) {
        if (_getOwner(poolId) != _DELEGATE_OWNER) {
            // Only the owner can perform "owner only" actions, unless the owner is delegated.
            return msg.sender == _getOwner(poolId);
        } else {
    ...
    function _getOwner(bytes32 poolId) internal view virtual returns (address);
